...
int InitString(String *&s){
	...
	s->str=(unsigned char *)calloc(STRINGSIZE,sizeof(char));
	if(s->str==NULL){
		return(2);
	}
	s->length=0;
...
int ClearString(String *&s){
	...
	if(s->size>STRINGSIZE){
		s->str=(unsigned char *)realloc(s->str,STRINGSIZE*sizeof(char));
		if(s->str==NULL){
		    return(1);
	    }
		s->size=STRINGSIZE;
...
int StringInsert(String *&s,String *&t,int i){
	...
	if(s->length+t->length>=s->size){
		...
		s->str=(unsigned char *)realloc(s->str,(s->size+add*STRINGADDON)*sizeof(char));
		if(s->str==NULL){
			return(2);
		}
...
int StringDelete(String *&s,String *&t,int i,int l){
	...
	if(l>=t->size){
		...
		t->str=(unsigned char *)realloc(t->str,(t->size+add*STRINGADDON)*sizeof(char));
		if(t->str==NULL){
			return(2);
		}
