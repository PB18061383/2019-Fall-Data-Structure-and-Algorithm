...
//这是用于单链线性表的基本操作，使用时请手动改ListElemType定义，并按需修改TraverseList函数。
//由于赶工繁忙，可能有些许bug，对此本人深感抱歉。如有建设性建议，可通过QQ：1345578933联系（麻治昊，20190918于中国科学技术大学）  
#include<stdio.h> 
#include<stdlib.h> 
//单链线性表数据类型，缺省int  
...
int GetElem(List *&l,int i,ListElemType &e){
	//返回单链表l第i个元素,成功将值赋值给e所在地址并返回0，失败返回1 
	int flag1;
	ListNode *flag2;
	if(i<1||i>l->length){
		return(1); 
...
int PriorElem(List *&l,ListElemType e,ListElemType &pe){
	//返回单链表第一个值为e的元素前驱，成功将赋值给pe所在地址并返回0，没有返回1 
	ListNode *flag;
	for(flag=l->p->next;flag->next!=NULL;flag=flag->next){
		if(flag->next->data==e){
			pe=flag->data;
			return(0);
		}
...
int NextElem(List *&l,ListElemType e,ListElemType &ne){
	//返回单链表第一个值为e的元素前驱，成功将赋值给ne所在地址并返回0，没有返回1 
	ListNode *flag;
	for(flag=l->p->next;flag->next->next!=NULL;flag=flag->next){
		if(flag->data==e){
			ne=flag->next->data;
			return(0);
		}
...
void TraverseList(List *&l){
	//遍历l的所有内容执行自定义操作 
	ListNode *flag;
	for(flag=l->p->next;flag!=NULL;flag=flag->next){
		//自定义操作，缺省打印
		printf("%d\n",flag->data); 
	}
	return;
